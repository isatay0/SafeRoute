...
const STORAGE_KEY = "saferoute_incidents_local";
...
window.deleteIncident = function(localId) {
...
async function buildRoutes() {
    ...
    if (!fast) {
        stats.innerHTML = "<b style='color:#b71c1c'>Route could not be built.</b>";
        return;
    }
...
async function fetchRoute(pts, profile) {
    ...
    try {
        ...
        return (d.routes && d.routes.length > 0)
            ? { poly: d.routes[0].geometry.coordinates, dist: d.routes[0].distance }
            : null;
    } catch {
        return null;
    }
...
function render(fast, safe, isAbsolute) {
    ...
    fastLayer = L.geoJSON(
        { type: "LineString", coordinates: fast.poly },
        { style: { color: "#ccc", weight: 3, opacity: 0.3 } }
    ).addTo(map);

    const routeColor = (isAbsolute && danger) ? "#b71c1c" : "#0d47a1";
    safeLayer = L.geoJSON(
        { type: "LineString", coordinates: safe.poly },
...
function resetRoute() {
    ...
    startMarker = endMarker = fastLayer = safeLayer = null;
